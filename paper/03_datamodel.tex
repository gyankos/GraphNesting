...
\section{Data Model}
The term \textit{property graph}  usually refers to a directed, labelled and attributed multigraph (each vertex and edge is represented as a relational tuple). In a property graph a single \textit{label}  is associated to every vertex and edge (e.g., \texttt{Author} or \texttt{coAuthorsip}). Further on, vertices and edges may have arbitrary named attributes (\textit{properties}) in the form of key-value pairs (e.g., \texttt{fullname:"Alice McKenzie"} or \texttt{institution:"Bengodi Ltd."}). Properties are set as tuples from the relational model. We define the\textit{ nested (property) graphs} as follows:
