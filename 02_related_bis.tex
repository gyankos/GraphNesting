...
As already mentioned before, some recent graph query languages can add graph nesting semantics to their graph data model. To express the query presented in our running example, these query languages must support id collections or nested representations.

For these reasons, we firstly select PostgreSQL's dialect which, by extending the SQL-3 syntax and by allowing JSON data, provides \texttt{array\_agg} aggregation function. The latter collects (i.e., groups) the result-set into arrays.  We  represent  graphs by  storing edge triples in the following form:
\begin{center}
