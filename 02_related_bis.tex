...
As already mentioned before, some recent graph query languages can add graph nesting semantics to their graph data model. To express query presented of our running example these query languages must either support id collections or nested representations.

For these reasons, we firstly select PostgreSQL's dialect which, by extending the SQL-3 syntax with JSON data, provides \texttt{array\_agg} aggregation function. The latter collects (i.e., groups) the result-set into arrays.  We  represent  graphs by  storing edge triples in the following form:
\begin{center}
 \texttt{Edge}(\textit{edgeId},\;\textit{sourceId},\;\textit{edgeLabel},\;\textit{targetId})
\end{center}
Nested vertices in our running example are obtained by 
grouping edges by \textit{sourceId} and collecting all target's ids results via \texttt{array\_agg}. Similarly, our nested edges are obtained by joining consecutive \texttt{Edge}s and then grouping  by two distinct \textit{sourceId}; the list of all the \textsc{Paper}s is collected via \texttt{array\_agg}. The overall graph nesting cannot be created in one single SQL query, because we cannot distinctively group the same dataset in different ways. Instead, we must perform two distinct aggregations (see Listing \ref{SQLNesting} in Appendix).
