...
In a property graph a single \textit{label}  is associated to every vertex and edge (e.g., \texttt{Author} or \texttt{coAuthorsip}). Further on, vertices and edges may have arbitrary named attributes (\textit{properties}) in the form of key-value pairs (e.g., \texttt{name:"Baldwin"} or \texttt{surname:"Oliver"}). In the presence of a schema Property-value associations of vertices and edges can be represented by relational tuples; this is a common approach in literature to associate data to both vertices and edges \cite{angles12,bergamimm16}. We define the\textit{ nested (property) graphs} as the following extension of the property graph data model for nested information:
