...
\parbox[t]{2mm}{\multirow{10}{*}{\rotatebox[origin=c]{90}{$\eta$ operator dependencies}}} & $dt$ & index dovetailing function \\
                                 ...
                                 & $g_\kappa(G_o)$ & nested graph classifier for $G_o$ over $\kappa$\\ 
                                 & $f_C(\kappa)$ & morphism between pattern $\kappa$ and subgraph $G_C$\\
                                 & $\iota_{G_o}(G_C)$ & indexing function for subgraphs $G_C\subseteq G_o$\\   
                                 & $\mu_\Omega(G_C)$ & object user defined function\\  
                                 & $\mu_E(G_C)$ & edge user defined function\\      
\bottomrule
\end{tabular}
\end{adjustbox}
