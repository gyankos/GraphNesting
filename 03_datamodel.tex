...
The simple edge $6$ in Figure \ref{fig:inputbibex2} ($\nu(0,6)=\epsilon(0,6)=\emptyset$) has id $(0,6)$ in the present model; it has one label, $\ell(0,6)=[\mstr{AuthorOf}]$ and it is associated to an empty tuple ($\omega(0,6)=\{\}$).
The source and target vertices in are 
$\lambda(0,6)=\Braket{(0,0),\;(0,3)}$. Similar considerations can be carried out for all the remaining edges.

The simple vertex $0$ in the same Figure has id $(0,0)$ in the present model; such vertex refers to the \mstr{Author} \texttt{Abigail Conner}. This information is represented as follows:
